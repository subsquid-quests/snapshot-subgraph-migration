...
exports.getBlockHeight = exports.getBlockName = exports.getTxHash = exports.toRpcDataRequest = exports.toQty = exports.qty2Int = exports.mapBlock = void 0;
const util_internal_1 = require("@subsquid/util-internal");
const assert_1 = __importDefault(require("assert"));
const util_1 = require("../util");
function mapBlock(block, transactionsRequested) {
    try {
...
function tryMapBlock(src, transactionsRequested) {
    let block = {
    ...
    if (transactionsRequested) {
        for (let i = 0; i < src.transactions.length; i++) {
            ...
            let tx;
            let id = (0, util_1.formatId)(block.header.height, block.header.hash, i);
            ...
            if (src._receipts) {
                ...
                (0, assert_1.default)(receipt.transactionHash === tx.hash);
    ...
    for (let log of iterateLogs(src)) {
        ...
        block.logs.push({
            id: (0, util_1.formatId)(block.header.height, block.header.hash, logIndex),
            logIndex,
            transactionIndex,
    ...
    if (src._traceReplays) {
        ...
        for (let rep of src._traceReplays) {
            let transactionIndex = (0, util_internal_1.assertNotNull)(hash2idx.get(rep.transactionHash));
            if (rep.trace) {
    ...
    if (src._debugFrames) {
        ...
        (0, assert_1.default)(src._debugFrames.length === src.transactions.length);
    ...
    if (src._debugStateDiffs) {
        ...
        (0, assert_1.default)(src._debugStateDiffs.length === src.transactions.length);
...
function mapBlockHeader(block) {
    let height = qty2Int(block.number);
    return {
        id: (0, util_1.formatId)(height, block.hash),
        height,
        hash: block.hash,
...
function mapTraceFrame(transactionIndex, src) {
    switch (src.type) {
        case 'create': {
            let rec = {
                transactionIndex,
        ...
        default:
            throw (0, util_internal_1.unexpectedCase)(src.type);
    }
}
function* iterateTraceStateDiffs(transactionIndex, stateDiff) {
    if (stateDiff == null)
        return;
...
function mapTraceStateDiff(transactionIndex, address, key, diff) {
    if (diff === '=') {
        return {
            transactionIndex,
            address,
...
function* mapDebugFrame(transactionIndex, debugFrameResult) {
        ...
        switch (rec.frame.type) {
            ...
            case 'CREATE2':
                yield {
                    ...
                    action: {
                        from: rec.frame.from,
                        value: BigInt((0, util_internal_1.assertNotNull)(rec.frame.value)),
            ...
            case 'SELFDESTRUCT':
                yield {
                    ...
                    action: {
                        ...
                        balance: BigInt((0, util_internal_1.assertNotNull)(rec.frame.value))
                    }
                };
                break;
            default:
                throw (0, util_internal_1.unexpectedCase)(rec.frame.type);
        }
    }
}
...
function* mapDebugStateDiff(transactionIndex, debugDiffResult) {
...
function* mapDebugDiff(transactionIndex, address, prev, next) {
    if (next.code) {
    ...
    if (next.nonce) {
        yield makeDiffRecord(transactionIndex, address, 'nonce', '0x' + prev.nonce?.toString(16), '0x' + next.nonce?.toString(16));
    }
...
function makeDiffRecord(transactionIndex, key, address, prev, next) {
    if (prev == null) {
        return {
            transactionIndex,
            address,
            key,
            kind: '+',
            next: (0, util_internal_1.assertNotNull)(next)
        };
    }
    if (next == null) {
        return {
...
exports.toRpcDataRequest = toRpcDataRequest;
...
function transactionRequired(req) {
    let f;
...
function receiptsRequested(req) {
    if (!transactionsRequested(req))
        return false;
